...
const AllSpecialist: React.FC<AllSpecialistProps> = ({
  token,
  specialist,
  getData,
  modalToggle,
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await axios.delete(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/specialists/${id}`,
        {
          headers: {
            method: 'DELETE',
      ...
      >
        <div className='grid w-full grid-cols-12 items-center justify-center gap-5'>
          ...
          <div className='col-span-10 grid grid-cols-3 items-center justify-between gap-5'>
            <p>{specialist.name}</p>
            <p>{specialist.designation}</p>
            <p>{specialist?.dateOfBirth}</p>
          </div>
